...
RULES_FILES=(./scripts/tests/output/sloth-rules/templates/*)
...
main() {
    ...
    for rulesFile in "${RULES_FILES[@]}" ; do
        ...
        while read -r alertname runbook severity overflow ; do
            ...
            if ! isInArray "$runbook" "${runbooks[@]}"; then
        ...
        done < <(yq -o json "$rulesFile" | jq -j '.spec.slos[] | .alerting.name, " ", .annotations.runbook_url, " ", .alerting.pageAlerts.labels.severity, "\n"')
